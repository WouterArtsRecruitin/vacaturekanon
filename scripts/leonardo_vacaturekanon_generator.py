...
class LeonardoGenerator:
    ...
    def generate_image(self, prompt: str, prompt_num: int) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Leonardo returns generations list
    ...
    def generate_batch(self, batch_size: int = 5, delay_between: float = 2.0):
        ...
        for batch_idx in range(0, len(VACATUREKANON_PROMPTS), batch_size):
            ...
            print(f"\n BATCH {batch_num}/{(len(VACATUREKANON_PROMPTS) - 1) // batch_size + 1}")
            print("-" * 60)
    ...
    def print_summary(self):
        ...
        print("="*60)
        print(f"Successful:  {len(self.results['success'])}/40")
        print(f"Failed:      {len(self.results['failed'])}/40")
        print(f"Success Rate:  {len(self.results['success'])/40*100:.1f}%")
        print("="*60)

        if self.results["failed"]:
            print("\nFailed prompts:")
            for failed in self.results["failed"]:
                print(f"  Prompt {failed['prompt_num']}: {failed['error']}")
...
def main():
    ...
    generator = LeonardoGenerator(LEONARDO_API_KEY)
    ...
    # Save results
    summary = generator.save_results("vacaturekanon_generation_results.json")
